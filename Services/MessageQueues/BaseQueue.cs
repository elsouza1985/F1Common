...
namespace FC1_CC.Common.Services.MessageQueues
{
    using System;
    ...
    public abstract class BaseQueue<T> : IBaseQueue<T>
    {
        ...
        public async Task SendAsync(IEnumerable<T> queueMessageContentBatch)
        {
            ...
            if (queueMessageContentBatchAsList.Count > MaxNumberOfMessagesInBatchRequest)
            {
        ...
        public async Task SendDelayedAsync(T queueMessageContent, double delayNumberOfSeconds)
        {
            ...
            var serviceBusMessage = new ServiceBusMessage(messageBody);
            await sender.ScheduleMessageAsync(serviceBusMessage, scheduledEnqueueTimeUtc);
        }
    }
}
