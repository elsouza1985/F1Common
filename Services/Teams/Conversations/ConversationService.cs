...
namespace FC1_CC.Common.Services.Teams
{
    ...
    public class ConversationService : IConversationService
    {
        ...
        public async Task<CreateConversationResponse> CreateUserConversationAsync(
            ...
            ILogger log)
        {
            var useCertificate = userAppCredentials.IsCertificateAuthenticationEnabled();
            return await CreateConversationAsync(teamsUserId, tenantId, serviceUrl, maxAttempts, userAppCredentials, useCertificate, log);
        }
        ...
        public async Task<CreateConversationResponse> CreateAuthorConversationAsync(
            ...
            ILogger log)
        {
            var useCertificate = authorAppCredentials.IsCertificateAuthenticationEnabled();
            return await CreateConversationAsync(teamsUserId, tenantId, serviceUrl, maxAttempts, authorAppCredentials, useCertificate, log);
        }
            ...
            ILogger log)
        {
            ...
            try
            {
                var retryPolicy = GetRetryPolicy(maxAttempts, log);
                if (useCertificate)
                {
                    await retryPolicy.ExecuteAsync(async () =>
                    await botAdapter.CreateConversationUsingCertificateAsync(
                        channelId: MicrosoftTeamsChannelId,
                ...
                {
                    await retryPolicy.ExecuteAsync(async () =>
                    await botAdapter.CreateConversationUsingSecretAsync(
                        channelId: MicrosoftTeamsChannelId,
                        serviceUrl: serviceUrl,
