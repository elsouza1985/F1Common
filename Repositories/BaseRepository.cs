        ...
        where T : TableEntity, new()
    {
        ...
        public async Task DeleteAsync(T entity)
        {
            try
            {
                ...
                entity = await GetAsync(partitionKey, rowKey);
                if (entity == null)
                {
        ...
        public async Task<IEnumerable<T>> GetWithFilterAsync(string filter, string partition = null, int? count = null)
        {
            try
            {
                var partitionKeyFilter = GetPartitionKeyFilter(partition);
                var combinedFilter = CombineFilters(filter, partitionKeyFilter);
                var query = new TableQuery<T>().Where(combinedFilter);
                var entities = await ExecuteQueryAsync(query, count);
                return entities;
            }
            catch (Exception ex)
            {
        ...
        public async Task<(IEnumerable<T>, TableContinuationToken)> GetPagedAsync(string partition = null, int? count = null, TableContinuationToken token = null)
        {
            ...
            if (token == null)
            {
                seg = await Table.ExecuteQuerySegmentedAsync(query, null);
        ...
        /// <summary>
        /// This method retrives a batch data entities
        /// </summary>
        /// <param name="notificationId">Notification ID</param>
        /// <returns>List of batch entities id:BatchNum</returns>
        public async Task<IEnumerable<T>> GetBatchesAsync(string notificationId)
        {
           // var filter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, notificationId);
            var entities = await this.GetAllAsync(notificationId);
            return entities;
        }
        ...
        private string GetPartitionKeyFilter(string partition)
        {
            var filter = TableQuery.GenerateFilterCondition(
                ...
                QueryComparisons.Equal,
                string.IsNullOrWhiteSpace(partition) ? defaultPartitionKey : partition);
            return filter;
        }
