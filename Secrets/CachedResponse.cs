...
namespace FC1_CC.Common.Secrets
{
    ...
    internal class CachedResponse : Response
    {
        ...
        /// </summary>
        internal bool IsValid => DateTimeOffset.Now <= expires;
        ...
        /// <inheritdoc/>
        protected override bool ContainsHeader(string name) => headers.Contains(name);

        /// <inheritdoc/>
        protected override IEnumerable<HttpHeader> EnumerateHeaders() => headers;

        /// <inheritdoc/>
        protected override bool TryGetHeader(string name, out string value) => headers.TryGetValue(name, out value);

        /// <inheritdoc/>
        protected override bool TryGetHeaderValues(string name, out IEnumerable<string> values) => headers.TryGetValues(name, out values);
